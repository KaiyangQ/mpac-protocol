...
- **Distributed validation demos** ([ref-impl/demo/distributed/](../ref-impl/demo/distributed/)) — WebSocket-based live runs in five domains: code-editing with optimistic concurrency control, family-trip planning with `task_set` scope overlap, a **coordination overhead benchmark** (MPAC eliminates coordination overhead -95% while preserving decision time), a **pre-commit + fault recovery** demo (OP_PROPOSE authorization, agent crash simulation, INTENT_CLAIM with governance approval), and a **conflict escalation** demo (CONFLICT_ESCALATE to arbiter, Claude-powered judicial resolution). All 21 message types have live Claude API demo coverage.
