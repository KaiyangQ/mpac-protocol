...
|---------------|------|----------|---------------------------|
| Dad (张伟) | Budget controller, driver | `agent:dad-planner` | Controls budget (total ¥15,000); prefers nature/outdoor; must drive ≤ 5 hours/day; wants to bring camping gear |
| Mom (李娜) | Food & accommodation lead | `agent:mom-planner` | Responsible for restaurant & hotel booking; wants cultural experiences; needs kid-friendly dining; allergic to seafood |
| Kid (张小明, 12 years old) | Activity lead | `agent:kid-planner` | Wants theme parks & water activities; school break July 10–25; needs at least 1 day of "free play" time |
...
```json
{
  ...
  "payload": {
    ...
    "objective": "Plan driving route and outdoor activities: Destination Moganshan (莫干山), 3.5hr drive from Shanghai, with hiking and camping on Days 1-2",
    "scope": {
      "kind": "task_set",
      "resources": [
        "itinerary://day-1",
...
```json
{
  ...
  "payload": {
    ...
    "objective": "Book kid-friendly hotel with cultural experience: Prefer boutique minsu (民宿) near Moganshan bamboo forest, with local cooking class on Day 2",
    "scope": {
      "kind": "task_set",
      "resources": [
        "itinerary://day-1",
...
```json
{
  ...
  "payload": {
    ...
    "summary": "Day 2: 8:00 campsite breakfast, 9:00 bamboo trail hike (Dad leads), 11:30 check out camp → drive to Moganshan Bamboo Villa (民宿), 12:30 lunch at minsu (local vegetable dishes, NO seafood), 14:30 local bamboo weaving class + cooking class (learn to make Moganshan smoked tofu), 18:00 farm-to-table dinner at minsu"
  }
}
```
...
| Day 2 | Campsite breakfast → bamboo trail hike | Check in Bamboo Villa (民宿), bamboo weaving + cooking class | Farm-to-table dinner at minsu | Moganshan Bamboo Villa | Mom's agent |
