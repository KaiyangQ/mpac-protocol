# MPAC Specification v0.1.15

## 1. Status

This document defines version `0.1.15` of the Multi-Principal Agent Coordination Protocol (`MPAC`).
...
#### 15.3.2 Cross-Principal Scope Race Lock (v0.1.15+)

When an `INTENT_ANNOUNCE` arrives whose `scope.resources` directly overlaps the resources of any **active** intent owned by a **different** principal, the session coordinator MUST reject it with a `PROTOCOL_ERROR` (`error_code`: `STALE_INTENT`). The rejected intent MUST NOT be registered in the coordinator's intent table, and the coordinator MUST NOT generate a `CONFLICT_REPORT` for the collision (the race lock pre-empts the advisory path that would otherwise fire under Section 17.5's `scope_overlap` category).

The rejection's `description` SHOULD identify both the colliding intent's `intent_id` and the holding `principal_id`, allowing the rejected client to call `INTENT_DEFERRED` (Section 15.5.1) with the correct `observed_intent_ids` rather than retrying blindly.

This rule applies ONLY to **direct resource overlap** (would-be `scope_overlap` per Section 17.5). It does NOT apply to cross-file `dependency_breakage` candidates, which remain advisory via `CONFLICT_REPORT`. The category split exists because:

- **Same-resource simultaneity** is almost always a real conflict — the second writer overwrites the first, and the user's expectation matches the merge-conflict semantics of source-control systems (resolve before proceeding).
- **Cross-file dependency overlap** is often backward-compatible (e.g., adding a keyword-only argument with a default value, adding a new method). Auto-rejecting every dependent whenever a hub file is touched would block legitimate parallel work and harm collaboration in large codebases. The advisory `CONFLICT_REPORT` path lets clients warn humans without blocking write paths.

**Race lock vs. intent backoff.** Section 15.3.1 (`INTENT_BACKOFF`) governs re-announcement after a previous conflict-driven `RESOLUTION` against the same participant. Section 15.3.2 (`STALE_INTENT`) governs first-time announcement when an active peer already holds the scope. They are independent mechanisms with independent error codes.

**Same-principal exemption.** Re-announcement by the **same** principal on the same scope is NOT subject to the race lock; it goes through the auto-supersede path described elsewhere (the prior intent is treated as an orphan from a crashed retry and replaced by the new announce, retaining the `intent_id` from the new request).

**Lock release.** The lock is bound to the holding intent's lifecycle. Whenever the holding intent reaches a terminal state (via `INTENT_WITHDRAW`, TTL expiry, supersede, transfer, or any other path defined in Section 15.6.1), the lock releases and a fresh `INTENT_ANNOUNCE` from any principal succeeds. The coordinator MUST NOT retain race-lock state beyond the holding intent's lifetime.

**Client recovery flow** (informative — clients are not required to follow this exact sequence):

1. Client receives `PROTOCOL_ERROR(STALE_INTENT)`.
2. Client extracts the colliding `intent_id` from the rejection's `description`.
3. Client sends `INTENT_DEFERRED` with `observed_intent_ids` referencing that id, signaling the yield.
4. Client surfaces the situation to its principal (e.g., "X is already editing this file; I yielded — say 'proceed anyway' to override").
5. If the principal overrides, the client MAY retry `INTENT_ANNOUNCE` once the holding intent transitions to a terminal state (signaled by `INTENT_WITHDRAW` broadcast or by the `INTENT_DEFERRED` resolution follow-up the coordinator emits per Section 15.5.1).
...
(a) **on every relevant intent-state transition** that could change the truth value (the standard reactive cleanup path); AND
(b) **at the moment the deferral is first registered** — if any condition is already true at arrival time (for example, the observed intent withdrew before the deferral arrived), the coordinator MUST emit `status: resolved` in the same response that delivers the active broadcast, rather than wait for a future state transition that will never come.

Without (b), a slow-yielding agent whose deferral arrives after its observed peer has already terminated would leave a stranded entry in the deferral table, and the corresponding UX hint would persist on sibling clients for the full TTL window despite the underlying coordination already being moot.
...
- `STALE_INTENT`: an `INTENT_ANNOUNCE` was rejected because at least one resource in its scope already overlaps the resources of an active intent owned by a **different** principal (Section 15.3.2). The lock pre-empts the advisory `CONFLICT_REPORT` path for would-be `scope_overlap` collisions; cross-file `dependency_breakage` candidates remain advisory
