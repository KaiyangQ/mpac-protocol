# MPAC Specification v0.1.8

## 1. Status

This document defines version `0.1.8` of the Multi-Principal Agent Coordination Protocol (`MPAC`).
...
1. **Pre-commit model** (`execution_model`: `pre_commit`): The agent proposes a mutation via `OP_PROPOSE` or `OP_COMMIT`, but the mutation is NOT applied to shared state until the session coordinator confirms acceptance. In this model, `OP_COMMIT` is a **request to commit** — the coordinator validates it (scope consistency, conflict check, governance approval if required) and either confirms it (the operation transitions to `COMMITTED`) or rejects it (via `OP_REJECT`). The agent MUST NOT modify shared state until confirmation is received. This model is RECOMMENDED for Governance Profile sessions and cross-organizational deployments where mutations must be reviewed before taking effect.
...
Key implications:
- In pre-commit model, `OP_REJECT` prevents the mutation from occurring. In post-commit model, `OP_REJECT` signals that the already-applied mutation is contested and may require rollback.
- In pre-commit model, the coordinator MUST send an explicit confirmation message (an `OP_COMMIT` echoed back with the coordinator as sender, or a dedicated acknowledgment — the mechanism is implementation-defined but MUST be unambiguous). In post-commit model, no confirmation is needed.
- `OP_BATCH_COMMIT` (Section 16.8) follows the same execution model as `OP_COMMIT`.
...
1. **Initialization**: Each participant MUST initialize a local Lamport counter to `0` upon joining a session (sending `HELLO`). The session coordinator MUST initialize its Lamport counter to `0` upon session creation, or restore it from the most recent snapshot upon recovery (Section 8.1.1.3).
...
6. **Monotonicity**: Lamport clock values in messages from the same sender MUST be strictly monotonically increasing. A receiver that detects a non-monotonic Lamport value from the same sender (a value less than or equal to a previously received value from that sender) SHOULD treat it as a potential replay or ordering violation and MAY reject the message with a `PROTOCOL_ERROR` (`error_code`: `MALFORMED_MESSAGE`).
...
| SUSPENDED | `INTENT_CLAIM` approved | claim authorized per governance | TRANSFERRED | original intent closed, new intent created as ACTIVE | coordinator + claimer |
...
When an intent transitions to a terminal state (`EXPIRED`, `WITHDRAWN`, or `SUPERSEDED`), the session coordinator MUST evaluate all operations that reference that intent via `intent_id`:
...
| (none) | `OP_COMMIT` received (pre-commit model) | sender registered, valid state_refs | PROPOSED | register as pending, await coordinator confirmation | proposer |
| PROPOSED | coordinator confirms (pre-commit model) | validation passed, no blocking conflict | COMMITTED | apply mutation, notify participants | coordinator |
...
| PROPOSED | referenced intent terminated | intent enters EXPIRED/WITHDRAWN/SUPERSEDED (Section 15.7) | REJECTED | system-generated OP_REJECT with reason `intent_terminated` | coordinator |
...
| (none) | `OP_BATCH_COMMIT` received | all entries valid per execution model | per entry: COMMITTED (post-commit) or PROPOSED (pre-commit) | register all entries atomically if `all_or_nothing` | proposer |

Terminal states: **REJECTED**, **ABANDONED**, **SUPERSEDED**. **COMMITTED** is a stable state (only exits via `OP_SUPERSEDE`).
...
- the `atomicity` field determines batch behavior:
  - `all_or_nothing`: all operations in the batch succeed or all fail. If any operation in the batch triggers a conflict or fails validation, the entire batch MUST be rejected. In pre-commit model (Section 7.8), the coordinator MUST validate all operations before confirming any. In post-commit model, if a conflict is detected post-facto, the compensating operation MUST reverse all operations in the batch.
...
| OPEN | `RESOLUTION` received | resolver has authority (owner/arbiter/coordinator) | RESOLVED | apply outcome, release frozen scope if any | resolver |
...
| ACKED | `RESOLUTION` received | resolver has authority | RESOLVED | apply outcome, release frozen scope if any | resolver |
...
| ESCALATED | `RESOLUTION` received | resolver has authority (typically arbiter) | RESOLVED | apply outcome, release frozen scope if any | resolver (typically escalation target) |
...
When all intents referenced by a conflict's `related_intents` field have transitioned to terminal states (`EXPIRED`, `WITHDRAWN`, or `SUPERSEDED`), and all operations referenced by the conflict's `related_ops` field are in terminal states (`REJECTED`, `ABANDONED`, or `SUPERSEDED`), the conflict SHOULD be automatically transitioned to `DISMISSED` by the session coordinator.
...
Concurrent resolution rule: If the session coordinator receives multiple `RESOLUTION` messages for the same `conflict_id`, the coordinator MUST accept only the first valid `RESOLUTION` received (determined by coordinator receipt order) and reject subsequent `RESOLUTION` messages for the same conflict with a `PROTOCOL_ERROR` (`error_code`: `RESOLUTION_CONFLICT`). This first-resolution-wins rule applies regardless of the resolver's role — whether the resolutions come from multiple `owner`-role participants, multiple `arbiter`-role participants, or a mix of roles. The ordering is determined by the coordinator's receipt order, not by message timestamps. This rule parallels the first-claim-wins semantics for `INTENT_CLAIM` (Section 14.7.4) and ensures deterministic conflict resolution without requiring inter-resolver coordination.
...
- pre-commit execution model RECOMMENDED (Section 7.8)
...
- explicit execution model (pre-commit or post-commit, declared per session, Section 7.8)
- intents with mandatory pre-execution declaration (Governance Profile), formal state transition tables, and livelock prevention via exponential backoff (Section 15.3.1)
- operations with required state references, atomic batch commit support (`OP_BATCH_COMMIT`), and formal state transition tables
- conflicts with standardized scope overlap rules, formal state transition tables, and progressive frozen scope degradation
- governance with deadlock prevention, three-phase frozen scope recovery, and deterministic concurrent resolution handling (first-resolution-wins, Section 18.4)
- causal context with baseline watermark interoperability, explicit Lamport clock maintenance rules, and causal gap detection behavior (Section 12.8)
- security with credential exchange, role verification, key distribution, watermark integrity, and coordinator accountability (Verified profile)
- failure recovery with concurrent claim resolution

Its central design claim is that collaborative agent systems become more interoperable, auditable, and governable when intent, mutation, conflict, and resolution are represented as explicit protocol messages rather than hidden inside application logic. The protocol provides security profiles with concrete trust establishment mechanisms — including coordinator accountability in the Verified profile — for deployments ranging from intra-team to cross-organizational, coordinator fault tolerance with snapshot-based recovery and audit log replay for production reliability, governance mechanisms to prevent deadlock between equal-authority principals with progressive degradation rather than binary timeout-then-reject, deterministic concurrent resolution handling ensuring that multiple resolvers for the same conflict produce a single authoritative outcome, an explicit consistency model declaring coordinator-serialized total order under normal operation, an execution model that disambiguates whether OP_COMMIT is a request or a notification, atomic batch commit for multi-resource operations, intent re-announce backoff to prevent livelock in repeated scope overlap scenarios, causal gap detection behavior to guide participants toward conservative decisions when their causal context is incomplete, recovery semantics to handle participant failure without orphaning in-flight work, operation supersession for safe post-commit revisions, semantic interoperability mechanisms (canonical resource URIs and standardized semantic matching output) to enable cross-kind scope overlap detection and assumption contradiction identification, session lifecycle management with transcript export for compliance, formal JSON Schema definitions for machine-enforceable wire format compatibility, normative state transition tables for all three lifecycle state machines, explicit Lamport clock maintenance rules, and payload schema definitions to ensure cross-implementation field-level compatibility.
